...
def run(filename):
    ...
    for command in commands:
        ...
        if op == 'push':
            ...
            tmp = make_translate(float(command['args'][0]), float(command['args'][1]), float(command['args'][2]))
            matrix_mult( stack[-1], tmp )
            stack[-1] = [ x[:] for x in tmp]
            tmp = []
        elif op == 'scale':
            tmp = make_scale(float(command['args'][0]), float(command['args'][1]), float(command['args'][2]))
            matrix_mult( stack[-1], tmp )
            stack[-1] = [ x[:] for x in tmp]
            tmp = []
        elif op == 'rotate':
            if command['args'][0] == 'x':
                tmp = make_rotX(command['args'][1])
            elif command['args'][0] == 'y':
                tmp = make_rotY(command['args'][1])
            else:
                tmp = make_rotZ(command['args'][1])
            matrix_mult( stack[-1], tmp )
            stack[-1] = [ x[:] for x in tmp]
            tmp = []
        elif op == 'sphere':
